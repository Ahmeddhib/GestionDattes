...
import { getSoldeBonAchatAction } from "@/actions/paiements-agriculteurs/get-solde-bon-achat.action";
import { createPaiementAction } from "@/actions/paiements-agriculteurs/create-paiement.action";
...
export function RecordPaiementDialog({ bonAchatId, numero }: RecordPaiementDialogProps) {
    ...
    async function loadSolde() {
        const result = await getSoldeBonAchatAction(bonAchatId);
        if (result.success && result.data) {
            setMontantRestant(result.data.montantRestant);
        }
    }
    ...
    async function onSubmit(data: FormData) {
        ...
        formData.append("bonAchatId", bonAchatId);
        ...
        const result = await createPaiementAction(formData);
        setLoading(false);

        if (result.success) {
            toast.success(t("finance.paiements.paiementEnregistre"));
            setOpen(false);
            form.reset();
            router.refresh();
        } else {
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogTrigger asChild>
                ...
                >
                    <HandCoins className="h-3.5 w-3.5 text-[#C17A2B]" />
                    {t("finance.paiements.enregistrerPaiement")}
                </Button>
            </DialogTrigger>
            <DialogContent className="rounded-lg sm:max-w-125 bg-white">
                <DialogHeader>
                    <DialogTitle className="text-[#3D1C00]">
                        {t("finance.paiements.nouveauPaiement")} — {numero}
                    </DialogTitle>
